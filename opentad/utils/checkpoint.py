...
def save_checkpoint(model, model_ema, optimizer, scheduler, epoch, args, work_dir=None):
    ...
    checkpoint_path = os.path.join(save_dir, f"epoch_{epoch}_{args.na}.pth")
...
def save_best_checkpoint(model, model_ema, epoch, work_dir=None):
    ...
    checkpoint_path = os.path.join(save_dir, f"best_%s.pth"%epoch)
    torch.save(save_states, checkpoint_path)

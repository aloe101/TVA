...
try:
    ...
    from mamba_ssm.modules.odual_branch import Mamba as ODB
    from mamba_ssm.modules.bi_dm import Mamba as bidm
    from mamba_ssm.modules.db_dm import Mamba as dbdm
    from mamba_ssm.modules.wo_bi_dm import Mamba as wobidm
    from mamba_ssm.modules.wo_db_dm import Mamba as wodbdm
    from mamba_ssm.modules.bssdm_md import Mamba as mdmamba
...
class MaskMambaBlock(nn.Module):
    def __init__(
        ...
        elif use_mamba_type == "only_dual_branch":
            self.mamba = ODB(n_embd, d_conv=kernel_size, expand=1, d_state=d_state)
        elif use_mamba_type == "bi_dm":
            self.mamba = bidm(n_embd, d_conv=kernel_size, expand=1, d_state=d_state)
        elif use_mamba_type == "db_dm":
            self.mamba = dbdm(n_embd, d_conv=kernel_size, expand=1, d_state=d_state)
        elif use_mamba_type == "wo_bi_dm":
            self.mamba = wobidm(n_embd, d_conv=kernel_size, expand=1, d_state=d_state)
        elif use_mamba_type == "wo_db_dm":
            self.mamba = wodbdm(n_embd, d_conv=kernel_size, expand=1, d_state=d_state)
        elif use_mamba_type == "md_mamba":
            self.mamba = mdmamba(n_embd, d_conv=kernel_size, expand=1, d_state=d_state)
        else:

...
class AnchorFreeHead(nn.Module):
    ...
    def losses(self, cls_pred, reg_pred, mask_list, points, gt_segments, gt_labels):
        ...
        cls_pred = torch.cat(cls_pred, dim=1)[valid_mask]       #3024 20
    ...
    def prepare_targets(self, points, gt_segments, gt_labels):
        concat_points = torch.cat(points, dim=0)        #1512 4
        ...
        for gt_segment, gt_label in zip(gt_segments, gt_labels):
            ...
            cls_targets = min_len_mask @ gt_label_one_hot       #1512 20
            # to prevent multiple GT actions with the same label and boundaries
            cls_targets.clamp_(min=0.0, max=1.0)
            # OK to use min_len_inds
            reg_targets = reg_targets[range(num_pts), min_len_inds]
            # normalization based on stride
            reg_targets = reg_targets / concat_points[:, 3, None]

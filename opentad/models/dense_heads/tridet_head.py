...
class TriDetHead(AnchorFreeHead):
    def __init__(
        ...
        # self.gfocal_loss = build_loss(loss.gfocal_loss)
    ...
    def forward_test(self, feat_list, mask_list, **kwargs):
        ...
        for l, (feat, mask) in enumerate(zip(feat_list, mask_list)):
            ...
            cls_pred.append(self.cls_head(cls_feat))
            reg_pred.append(F.relu(self.scale[l](self.reg_head(reg_feat))))
            cls_start_pred.append(self.cls_start_head(cls_start_feat))    #list, 1, 20,
            cls_end_pred.append(self.cls_end_head(cls_end_feat))
    ...
    def pad_and_stride(self, cls_level_i, pad_left, num_bins):
        ...
        x_stride = list(x.stride())     #[C*(t_I+BINS), (t_I+BINS),1,1]
    ...
    def prepare_out_logits(self, out_start, out_end):
        ...
        return out_start_logits, out_end_logits  # list([B T_i C, num_bins+1])
    ...
    def losses(self, cls_pred, reg_pred, mask_list, points, gt_segments, gt_labels, out_start, out_end):
        ...
        gt_cls, gt_reg = self.prepare_targets(points, gt_segments, gt_labels)       #[(2,2), (2,22)]->[(4536,2), ...] [2,22]->[(4536,#class), ...]
        
        # positive mask
        gt_cls = torch.stack(gt_cls)  # [B,sum(T),num_classes]
        gt_reg = torch.stack(gt_reg)  # [B,sum(T),2]
        valid_mask = torch.cat(mask_list, dim=1)    #2,4536
        pos_mask = torch.logical_and((gt_cls.sum(-1) > 0), valid_mask)      #2,4536
        num_pos = pos_mask.sum().item()
        # cls_indices = torch.cat(gt_labels)
        # one_hot_cls = [0]*gt_cls.shape[-1]
        ...
        decoded_offsets = self.decode_offset(reg_pred, out_start_logits, out_end_logits)        # B 4536 20 2
        if self.mse_loss:
            mask_reg = decoded_offsets * gt_cls.unsqueeze(-1)     # B 4536 20 2
            mse_offset = mask_reg.max(dim=-2)[0]     #2 4536 2
            reg_gt_mask = gt_reg.unsqueeze(-2) * gt_cls.unsqueeze(-1)     #2 4536 20 2
            reg_gt_mask = reg_gt_mask.max(dim=-2)[0]     #2 4536 2
            mse_loss = self.mse(mse_offset, reg_gt_mask)
            mse_loss /= loss_normalizer
            penalty_offsets = decoded_offsets[~pos_mask]
            penalty_gt = torch.zeros_like(penalty_offsets)
            penalty_loss = nn.MSELoss(reduction="mean")(penalty_offsets, penalty_gt)
            penalty_loss /= loss_normalizer
        decoded_offsets = decoded_offsets[pos_mask]
        # masked_offsets = decoded_offsets[pos_mask]                                             # #gt 20 2 
        # decoded_offsets = decoded_offsets.max(dim=2)[0]                                       # B 4536 2
        # decoded_offsets = decoded_offsets.max(dim=0)[0]                                            # 4536 2
        


        # the boundary head predicts the classification score for each categories.
        # pred_offsets = decoded_offsets[:,:,one_hot_cls,:]
        pred_offsets = decoded_offsets[gt_cls[pos_mask].bool()]
        # prop_offsets = pred_offsets.view(-1,2)
        # pred_offsets = masked_offsets[gt_cls[pos_mask].bool()]                                 # #gt 2
        gt_reg = gt_reg[pos_mask][torch.where(gt_cls[pos_mask])[0]]
        # gt_prop = gt_reg.view(-1,2)
        # no_mask_points = points
        # points = torch.cat(points, dim=0).unsqueeze(0).repeat(pos_mask.shape[0], 1, 1)        # B 4536 4
        # pred_points = torch.cat(points, dim=0).repeat(int(prop_offsets.shape[0]/pred_offsets.shape[1]), 1)
        ...
        # gt_points = torch.cat(points, dim=0).repeat(int(gt_prop.shape[0]/gt_reg.shape[1]), 1)
        # no_mask_points = points.unsqueeze(-2).unsqueeze(0).repeat(2, 1, 20, 1)

        ## couple the classification loss with iou score
        pred_segments = self.get_proposals(points, pred_offsets)
        # no_mask_segments = self.get_proposals(no_mask_points, decoded_offsets)
        gt_segments = self.get_proposals(points, gt_reg)

        iou_rate = self.iou_loss(pred_segments, gt_segments, reduction="none")
        # no_mask_iou_rate = self.iou_loss(no_mask_segments, gt_segments, reduction="none")
        rated_mask = gt_target > self.label_smoothing / (self.num_classes + 1)
        cls_loss[rated_mask] *= (1 - iou_rate) ** self.iou_weight_power
        # no_mask_cls_loss *= (1 - no_mask_iou_rate) ** self.iou_weight_power

        cls_loss = cls_loss.sum()
        cls_loss /= self.loss_normalizer
        # no_mask_cls_loss = no_mask_cls_loss.sum()
        # no_mask_cls_loss /= self.loss_normalizer
        ...
        if num_pos == 0:
            ...
            # no_mask_reg_loss = self.reg_loss(no_mask_segments, gt_segments, reduction="sum")
            # no_mask_reg_loss /= loss_normalizer
        ...
        if self.mse_loss:
            return {"cls_loss": cls_loss, "reg_loss": reg_loss * loss_weight, "mse_loss": mse_loss, "penalty_loss": penalty_loss}
        else:

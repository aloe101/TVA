...
# from ..bricks import TransformerBlock


class SGPMDBlock(nn.Module):
    """
    ...
    def __init__(
        ...
        self.psi = DBM(n_embd, d_conv=4, use_fast_path=True, expand=1)
        self.fc = nn.Conv1d(n_embd, n_embd, 1, stride=1, padding=0, groups=n_embd)
        # self.fc = DBM(n_embd, d_conv=1, use_fast_path=True, expand=1)
        # self.convw = nn.Conv1d(n_embd, n_embd, kernel_size, stride=1, padding=kernel_size // 2, groups=n_embd)
        self.convw = DBM(n_embd, d_conv=4, use_fast_path=True, expand=1)
        # self.convkw = nn.Conv1d(n_embd, n_embd, up_size, stride=1, padding=up_size // 2, groups=n_embd)
        self.convkw = DBM(n_embd, d_conv=4, use_fast_path=True, expand=1)
        self.global_fc = nn.Conv1d(n_embd, n_embd, 1, stride=1, padding=0, groups=n_embd)
        # self.global_fc = DBM(n_embd, d_conv=1, use_fast_path=True, expand=1)
        ...
        # if self.mamba:
        self.mamba_block = DBM(n_embd, d_conv=4, use_fast_path=True, expand=1)
    ...
    def forward(self, x, mask):     # x: 2 512 2304
        B, C, T = x.shape
        x = self.downsample(x)
        ...
        # gf = self.mamba_block(out).permute(0, 2, 1)
        psi = self.psi(out)
        fc = self.fc(out.permute(0, 2, 1)).permute(0, 2, 1)
        convw = self.convw(out)
        convkw = self.convkw(out)
        phi = torch.relu(self.global_fc(out.permute(0, 2, 1).mean(dim=-1, keepdim=True))).permute(0, 2, 1)    # 2 512 1
        out = fc * phi + (convw + convkw) * psi + out
        out = out.permute(0, 2, 1)
        # out = gf * phi + out.permute(0, 2, 1)
        # if self.mamba:
        #     global_level = self.mamba_block(out.transpose(1, 2)).transpose(1, 2)
        #     out = out + global_level
        # if self.trans:
        #     global_level, out_mask = self.trans_block(out, out_mask)
        #     out = out + global_level

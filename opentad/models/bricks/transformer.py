...
class TransformerBlock(nn.Module):
    ...
    def __init__(
        ...
        self.mlp = nn.Sequential(
            ...
            act_layer(),
            nn.Dropout(proj_pdrop, inplace=False),
...
class MaskedMHCA(nn.Module):
    ...
    def forward(self, x, mask):
        ...
        k = k.view(B, self.n_head, self.n_channels, -1).transpose(2, 3)     #2 4 192 128
        q = q.view(B, self.n_head, self.n_channels, -1).transpose(2, 3)
        v = v.view(B, self.n_head, self.n_channels, -1).transpose(2, 3)

        # self-attention: (B, nh, T', hs) x (B, nh, hs, T'') -> (B, nh, T', T'')
        att = (q * self.scale) @ k.transpose(-2, -1)                        #2 4 192 192
        ...
        out = att @ (v * kv_mask[:, None, :, None].to(v.dtype))         #2 4 192 128
        # re-assemble all head outputs side by side
        out = out.transpose(2, 3).contiguous().view(B, C, -1)           #2 512 192

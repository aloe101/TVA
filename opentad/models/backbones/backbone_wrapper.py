...
class BackboneWrapper(nn.Module):
    def __init__(self, cfg):
        ...
        # new_state_dict = {k.replace("backbone", "model.backbone"): v for k, v in state_dict.items()}
        ...
        if hasattr(custom_cfg, "post_processing_pipeline_embedding"):
            self.post_processing_pipeline_embedding = Compose(custom_cfg.post_processing_pipeline_embedding)
        else:
            self.post_processing_pipeline_embedding = None
    ...
    def forward(self, frames, masks=None):
        ...
        if self.pre_processing_pipeline is not None:
            frames = self.pre_processing_pipeline(dict(frames=frames))["frames"]        #96 1 3 16 160 160
        ...
        if self.freeze_backbone:  # freeze everything even in training
            ...
            features = self.model.backbone(frames)  #, embedding
        #features.shape: 48, 768, 8, 10, 10
        ...
        #         features = self.post_processing_pipeline(dict(feats=embedding))["feats"]     #2 384 768
        
        # apply mask
        if masks is not None and features.dim() == 3:
            masks_safe = masks.unsqueeze(1).detach().float().expand_as(features)
            features = features * masks_safe  # 确保不发生 in-place

        # make sure detector has the float32 input
        features = features.to(torch.float32)
        # print(f'features shape: {features.shape}; embedding shape: {embedding.shape}')
        # if return_embedding:
        #     embedding = self.post_processing_pipeline_embedding(dict(feats=embedding))["feats"]
        #     return embedding
        # else:
    ...
    def unflatten_and_pool_features(self, features, batches, num_segs):
        ...
        # convert the feature to [B,C,T]: pooling and other operations
        if self.post_processing_pipeline is not None:
            features = self.post_processing_pipeline(dict(feats=features))["feats"]     #2 384 768
        return features

...
try:
    ...
    from mamba_ssm.modules.bssdm import Mamba as bssdm
...
class HybridAdapter(BaseModule):
    def __init__(
        self,
        embed_dims: int,
        ...
        self.mamba = MaskMambaBlock(hidden_dims, **mamba_cfg)
    ...
    def forward(self, x: Tensor, h: int, w: int) -> Tensor:
        ...
        # temporal depth-wise convolution
        B, N, C = x.shape  # 48, 8*10*10, 384       new:96 800 96
        attn = x.reshape(-1, self.temporal_size, h, w, x.shape[-1])  # [b,t,h,w,c]  [1,384,10,10,384]   2 384 10 10 96
...
class MaskMambaBlock(nn.Module):
    def __init__(
        ...
        elif use_mamba_type == "bssdm":
            self.mamba = bssdm(n_embd, d_conv=kernel_size, expand=1, d_state=d_state)
        else:
    ...
    def forward(self, x):
        ...
        x = res + self.drop_path(x_.transpose(1, 2))
...
class MambaAdapter(BaseModule):
    def __init__(
        ...
        if drop_path_rate_out > 0.0:
            self.drop_path = AffineDropPath(hidden_dims, drop_prob=drop_path_rate_out, transpose=True)
        else:
            self.drop_path = nn.Identity()
        trunc_normal_init(self.down_proj, std=0.02, bias=0)
        constant_init(self.up_proj, 0)  # the last projection layer is initialized to 0
    ...
    def forward(self, x: Tensor, h: int, w: int) -> Tensor:
        ...
        # temporal depth-wise convolution
        B, N, C = x.shape  # 48, 8*10*10, 384       new:96 800 96
        mamba_feat = x.reshape(-1, self.temporal_size, h, w, x.shape[-1])  # [b,t,h,w,c]  [1,384,10,10,384]   2 384 10 10 96
        mamba_feat = mamba_feat.permute(0, 2, 3, 4, 1).flatten(0, 2)  # [b*h*w,c,t] [1*10*10,384,384]
        # attn = self.dwconv(attn)  # [b*h*w,c,t] [1*10*10,384,384]
        # attn = self.conv(attn)  # [b*h*w,c,t] [1*10*10,384,384]
        mamba_feat = self.mamba(mamba_feat)
        mamba_feat = mamba_feat.unflatten(0, (-1, h, w)).permute(0, 4, 1, 2, 3)  # [b,t,h,w,c] [1,384,10,10,384]
        mamba_feat = mamba_feat.reshape(B, N, C)
        
        x = x + self.drop_path(mamba_feat) + res
...
class Attention(BaseModule):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        # fast attention
        x = F.scaled_dot_product_attention(q, k, v, dropout_p=self.attn_drop.p)     #96 6 800 64
...
class Block(BaseModule):
    ...
    def __init__(
        ...
        use_mamba_adapter: bool = False,
        use_standard_adapter: bool = False,
        drop_path_rate_out: float = 0.0,
        act_type: str = "gelu",
        mamba_cfg=dict(kernel_size=4, drop_path_rate=0.2, use_mamba_type="dbm"),
        **kwargs,
    ) -> None:
        super().__init__(init_cfg=init_cfg)
        ...
        self.use_hybrid_adapter = use_hybrid_adapter
        self.use_mamba_adapter = use_mamba_adapter
        self.use_standard_adapter = use_standard_adapter
        ...
        if self.use_adapter:
            ...
            elif self.use_mamba_adapter:
                self.adapter = MambaAdapter(
                    embed_dims=embed_dims,
                    kernel_size=3,
                    dilation=1,
                    temporal_size=temporal_size,
                    mamba_cfg=mamba_cfg,
                    drop_path_rate_out=drop_path_rate_out,
                    act_type=act_type,
                )
            elif self.use_standard_adapter:
...
class VisionTransformerAdapter(BaseModule):
        ...
        self.blocks = ModuleList(
            [
                Block(
                    ...
                    use_hybrid_adapter=use_hybrid_adapter,
                    use_mamba_adapter=use_mamba_adapter,
                    use_standard_adapter=use_standard_adapter,
                    drop_path_rate_out=drop_path_rate_out,
                    act_type=act_type,
                    mamba_cfg=mamba_cfg,
                )
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        Args:
            x (Tensor): The input data. bs96 800 embed_dims384
        Returns:
        ...
        if (h, w) != self.grid_size:
            pos_embed = self.pos_embed.reshape(-1, *self.grid_size, self.embed_dims)        #8 14 14 384  self.pos_embed:1 1568 384
            pos_embed = pos_embed.permute(0, 3, 1, 2)
            pos_embed = F.interpolate(pos_embed, size=(h, w), mode="bicubic", align_corners=False)
            pos_embed = pos_embed.permute(0, 2, 3, 1).flatten(1, 2)                         #8 100 384

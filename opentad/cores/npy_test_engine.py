...
def npy_eval_one_epoch(
    ...
    for data_dict in tqdm.tqdm(test_loader, disable=(rank != 0)):
        if file_path.endswith(".npy"):
            perturb = np.load(file_path)
            perturb = torch.from_numpy(perturb).to(data_dict["inputs"].device)
            data_input = data_dict['inputs'] + perturb
        elif file_path == 'none':
            data_input=data_dict['inputs']
        else:
            data_input = []
            for item in data_dict['metas']:
                if args.thumos:
                    data = np.load(os.path.join(file_path, f"adv_{item['video_name']}_{item['window_start_frame']}.npy"))
                else:
                    data = np.load(os.path.join(file_path, f"adv_{item['video_name']}_{item['resize_length']}.npy"))
                data_input.append(data)
            data_input = np.array(data_input)
            data_input = torch.from_numpy(data_input).to(data_dict["inputs"].device)
        if args.thumos and args.max_sample is not None:
            vid += [x['video_name'] for x in data_dict['metas']]
            start_frame += [x['window_start_frame'] for x in data_dict['metas']]  #['window_start_frame']
        elif args.max_sample is not None:
            vid += [x['video_name'] for x in data_dict['metas']]
        
        with torch.no_grad():
            results = model(
                    data_input,
                    data_dict['masks'], data_dict['metas'], #data_dict['gt_segments'], data_dict['gt_labels'],
                    return_loss=False,
    ...
    if rank == 0:
        ...
        if not not_eval:
            if args.thumos and args.max_sample is not None:
                filt_gt = dict(zip(vid, start_frame))
                evaluator = build_evaluator(dict(prediction_filename=result_eval, filt_gt=filt_gt, dataset=args.dataset, **cfg.evaluation))
                # evaluator_clean = build_evaluator(dict(prediction_filename=results_eval_clean, filt_gt=filt_gt, **cfg.evaluation))
                metrics_dict = evaluator.evaluate()
                # metrics_dict_clean = evaluator_clean.evaluate()
                evaluator.logging(logger)
            elif args.max_sample is not None:
                filt_gt = {v: 0 for v in vid}  # create a filter for anet
                evaluator = build_evaluator(dict(prediction_filename=result_eval, filt_gt=filt_gt, thumos=args.thumos, **cfg.evaluation))
                # evaluator_clean = build_evaluator(dict(prediction_filename=results_eval_clean, filt_gt=filt_gt, **cfg.evaluation))
                metrics_dict = evaluator.evaluate()
                # metrics_dict_clean = evaluator_clean.evaluate()
                evaluator.logging(logger)

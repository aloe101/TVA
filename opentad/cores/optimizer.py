...
def build_optimizer(cfg, model, logger):
    ...
    # print(cfg.keys())
    logger.info(f"model.module.projection.freeze_detector: {model.module.projection.freeze_detector}")
    if model.module.projection.freeze_detector == False:
        optim_groups = backbone_optim_groups + det_optim_groups
    else:
        optim_groups = backbone_optim_groups
    ...
    #     optimizer = torch.optim.RMSprop(optim_groups, **cfg)
    # elif optimizer_type == "AdaD":
    #     optimizer = torch.optim.Adadelta(optim_groups, **cfg)
    else:
        raise f"Optimizer {optimizer_type} is not supported so far."

    return optimizer

def build_backbone_optimizer(cfg, model, logger):
    optimizer_type = cfg["type"]
    cfg.pop("type")
    ...
    backbone_cfg = cfg["backbone"]
    cfg.pop("backbone")
    backbone_optim_groups = get_backbone_optim_groups_(backbone_cfg, model, logger)
...
def get_backbone_optim_groups(cfg, model, logger):
    ...
    for name, param in model.module.backbone.named_parameters():     #model.module.named_parameters():     #
...
def get_backbone_optim_groups_(cfg, model, logger):
    """Example:
    backbone = dict(
        lr=1e-5,
        weight_decay=1e-4,
